#!/bin/sh
# This script launches Execution Clients for Hyperdrive's docker stack; only edit if you know what you're doing ;)
...
if [ "$CLIENT" = "nethermind" ]; then
    ...
    if [ ! -z "$HD_NETHERMIND_ADDITIONAL_MODULES" ]; then
        HD_NETHERMIND_ADDITIONAL_MODULES=",${HD_NETHERMIND_ADDITIONAL_MODULES}"
    fi
    CMD="$CMD --JsonRpc.EnabledModules Eth,Net,Web3$HD_NETHERMIND_ADDITIONAL_MODULES"

    # Add optional supplemental JSON-RPC URLs
    if [ ! -z "$HD_NETHERMIND_ADDITIONAL_URLS" ]; then
        HD_NETHERMIND_ADDITIONAL_URLS=",${HD_NETHERMIND_ADDITIONAL_URLS}"
    fi
